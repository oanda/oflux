...
let emit_node_atomic_structs pluginopt readonly symtable aliases code =
	...
	let check_ptrs ntl = List.map 
		(fun (n,t,_) -> "static oflux::InsistOnIsPtr<"^t^"> "
...
let emit_atom_fill pluginopt symtable aliases code =
	...
	let code_for_one (cl,i) (n,t) =
                ...
                in
		(("oflux::atomic::Atomic * "^atomic_n^" = ah->get("
		^(string_of_int i)^",oflux::atomic::AtomicsHolder::HA_get_lexical)->atomic();")
	...
	let e_one n nd code =
		...
		with Not_found ->
			...
			let code = List.fold_left add_code code
				[ "void "^(trim_dot n)^"_atoms::fill(oflux::atomic::AtomicsHolder * ah)"
				; "{"
				] in
...
let emit_atom_map_decl symtable code =
	let e_one n gd code =
		...
		in  add_code code ("extern oflux::atomic::AtomicMapAbstract * "^clean_n
                                ^"_map_ptr;") in
...
let emit_atom_map_map plugin_opt symtable code =
	...
	let e_map_decl_lf n gd code =
		...
		("static "^(
                if gd.gtype = "pool" then
                        ("oflux::lockfree::atomic::AtomicPool "^clean_n^"_map; "
	...
	let e_map_ptrs n gd code =
                ...
                let clean_n = clean_dots n 
		in  add_code code 
			( "oflux::atomic::AtomicMapAbstract * "^clean_n
                                ^"_map_ptr = NULL;" ) in
	...
	let code = if with_code then
			let code =
				...
				let proto = "bool "^gtfunc^"( "
					^"void * out, const void *in" 
					^", oflux::atomic::AtomicsHolderAbstract * ah)"
				in  List.fold_left add_code code
					[ proto
					; "{"
					; "return true;"
...
let emit_test_main code =
	List.fold_left add_code code
		...
		; "{"
		; "assert(argc >= 2);"
		; "static oflux::flow::FunctionMaps ffmaps(ofluximpl::__conditional_map, ofluximpl::__master_create_map, ofluximpl::__master_create_door_map, ofluximpl::__theGuardTransMap, ofluximpl::__atomic_map_map, ofluximpl::__ioconverter_map);"
		; "oflux::DirPluginSource dir_plugin_src(\"xml\");"
		; "oflux::RunTimeConfiguration rtc = {"
		; "  1024*1024 // stack size"
		; ", 4 // initial threads (lock-free runtime uses this)"
		; ", 0 // max threads"
		; ", 0 // max detached"
		...
		; "};"
		; "oflux::logging::toStream(std::cout); // comment out if no oflux logging is desired"
		; "oflux::EnvironmentVar env(oflux::runtime::Factory::"^(CmdLine.get_runtime_engine())^");"
