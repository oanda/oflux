...
public:
	...
	~LinkedList()
		{
			N * n = _head;
			while(n) {
				N * nn = static_cast<N *> (n->next());
				N::destroy(n);
				n = nn;
			}
		}
	inline void insert_front_all(LinkedList<C,N> & ll)
		{ // not super efficient O(n)
			if(_head == NULL) {
				_head = ll._head;
				ll._head = NULL;
			} else {
	...
	inline bool isEmpty() { return _head == NULL; }
	inline bool isSingleton() { return _head != NULL && _head->next() == NULL; }
	inline void clear()
		{
			N * n = _head;
			while(n) {
...
public:
	LinkedListRemovable()
		...
		, _tail(NULL)
		{}
	inline void insert_front_all(LinkedListRemovable<C,N> & ll)
		{
			if(ll._head != NULL) {
				*(ll._tail) = this->_head;
				if(this->_head) {
					this->_head->previous(ll._tail);
				} else {
					_tail = ll._tail;
				}
				this->_head = ll._head;
	...
	inline void insert_back_all(LinkedListRemovable<C,N> & ll)
		{
			if(ll._head != NULL) {
				if(_tail) {
					*_tail = ll._head;
					ll._head->previous(_tail);
				} else {
					this->_head = ll._head;
					ll._head->previous(&(this->_head));
				}
				_tail = ll._tail;
				ll._head = NULL;
				ll._tail = NULL;
