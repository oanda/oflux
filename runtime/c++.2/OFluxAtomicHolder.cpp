...
int AtomicsHolder::acquire(AtomicsHolder & given_atomics, const void * node_in)
{
...
void AtomicsHolder::release( EventList & released_events)
{
	// reverse order
	for(int i = _number-1; i >= 0; i--) {
		...
		if(ha->haveit()) {
			Atomic * a = ha->atomic();
			boost::shared_ptr<EventBase> ev = a->release();
			if(ev != NULL) {
				released_events.insert_back(ev);
			}
		}
	}
}
