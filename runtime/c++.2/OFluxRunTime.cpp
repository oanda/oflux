...
void RunTimeConfiguration::init_RunTimeConfiguration()
{
	stack_size = (1024*1024);
	initial_thread_pool_size = (0);
	max_thread_pool_size = (0);
	max_detached_threads = (0);
	min_waiting_thread_collect = (0);
	thread_collection_sample_period = (0);
	flow_filename = (NULL);
	flow_maps = (NULL);
	prioritize_queued_sources = (false);
}
...
RunTime::RunTime(const RunTimeConfiguration & rtc)
	...
	, _queue(rtc.prioritize_queued_sources)
	, _running(false)
...
void RunTime::load_flow(const char * flname)
{
	...
	Removable<FlowNode> * source_n = flow->sources().first();
	while(source_n != NULL) {
		FlowNode * fn = source_n->content();
		CreateNodeFn createfn = fn->getCreateFn();
		boost::shared_ptr<EventBase> ev = (*createfn)(EventBase::no_event,fn);
		_queue.push(ev, true /*is a source*/);
		source_n = source_n->next();
	}
	while(_active_flows.size() > 0) {
		Flow * back = _active_flows.back();
		if(back->sources().first() != NULL) {
...
void RunTimeThread::start()
{
	...
	while(_system_running && !_request_death) {
		if(_rt->_waiting_to_run.count() > 0) { // no need to be fair
...
{
	LinkedList<FlowNode> fsuccessors;
	_flow_node_working = ev->flow_node();
	// ------------------ Guards --------------------
	static AtomicsHolder empty_ah;
	Atomic * must_wait_on_atomic = ev->acquire(empty_ah);
	if(must_wait_on_atomic) {
		must_wait_on_atomic->wait(ev);
	} else {
	// ---------------- Execution -------------------
		int return_code;
		{ 
...
#endif
				{
					...
					while(loop_on_one) {
						...
						ev->flow_node()->get_successors(fsuccessors, 
							ev->output_type(), return_code);
						Node<FlowNode> * fsucc_first = fsuccessors.first();
						FlowNode * fsucc_first_content = (fsucc_first ? fsucc_first->content() : NULL);
						if( fsucc_first != NULL
								&& fsucc_first->next() == NULL 
								&& fsucc_first_content->guards().isEmpty()
								&& fsucc_first_content->getIsDetached()
								&& ev->atomics().number() == 0) { 
							CreateNodeFn createfn = fsucc_first_content->getCreateFn();
							/*log_info("melding two detached nodes %s -> %s (thread %d)\n", 
								_flow_node_working->getName(),
								fsucc_first_content->getName(),
								oflux_self()
								);*/
							boost::shared_ptr<EventBase> new_ev =
								( fsucc_first_content->getIsSource()
								? (*createfn)(EventBase::no_event,fsucc_first_content)
								: (*createfn)(boost::shared_ptr<EventBase>(ev),fsucc_first_content)
								);
							...
							loop_on_one = false;
							/*log_info("failed to meld detached nodes %s -> %s (%s)\n", 
								_flow_node_working->getName(),
								(fsucc_first_content ? fsucc_first_content->getName() : "<none>"),
								  (ev->atomics().number() ? "had atomics"
								: (fsucc_first_content == NULL ? "no succ"
								: (!fsucc_first_content->getIsDetached() ? "succ not detached"
								: (!fsucc_first_content->guards().isEmpty() ? "succ has atomics"
								: "ok"))))
								);*/
						}
					}
	...
	// ----------- Successor processing -------------
		...
		EventList successor_events;
		//ev->successors(successor_events,return_code);
                {
			Node<FlowNode> * fsucc_n = fsuccessors.first();
			while(fsucc_n != NULL) {
				FlowNode * fn = fsucc_n->content();
				CreateNodeFn createfn = fn->getCreateFn();
				boost::shared_ptr<EventBase> ev_succ = 
					( fn->getIsSource()
					? (*createfn)(EventBase::no_event,fn)
					: (*createfn)(boost::shared_ptr<EventBase>(ev),fn)
					);
